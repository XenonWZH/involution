...
const char* courseName[] = {"Chinese", "Mathematics", "English", "Programming"};
...
void RemoveQuery(bool isQuery) {
    ...
    while (true) {
        ...
        if (!isQuery) printf("%d student(s) removed.\n", r);
    }
}

double getCourseStat(int c, int s, int* passed, int* failed) {
    int tot = 0;
    *passed = *failed = 0;
    for (int i = 0; i < n; i++)
...
void getOverallStat(int c, int* cnt) {
...
void stat() {
    ...
    for (int i = 0; i < 4; i++) {
        int passed, failed;
        double avg = getCourseStat(c, i, &passed, &failed);
        printf("%s\n", courseName[i]);
...
int main() {
    while (true) {
        ...
        if (choice == 0) break;
        if (choice == 1) add();
        if (choice == 2) RemoveQuery(false);
        if (choice == 3) RemoveQuery(true);
        if (choice == 4)
            printf(
                "Showing the ranklist hurts students' self-esteem. Don't do "
                "that.\n");
        if (choice == 5) stat();
    }
    return 0;
